...
class Preprocessor:
    ...
    def transform(
        ...
        words, postags, heads, deprels = zip(
            *[(token["form"], token["upos"], token["head"], token["deprel"]) for token in tokens]
        )
        deprels = [s.split(":")[0] for s in deprels]
        sample = (
...
def create_dataloader(
    ...
    **kwargs,
    ...
    dataset = _wrap_cache(
        lambda f: Dataset(map(preprocessor.transform, utils.conllu.read_conll(f))), file, cache_dir
    )
