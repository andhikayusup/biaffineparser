...
class EvaluateCallback(utils.training.Callback):
    ...
    def on_loop_end(self, context, metrics):
        ...
        with NamedTemporaryFile(mode="w") as f:
            utils.conllu.dump_conll(self._yield_prediction(), f)
            self.result.update(utils.conllu.evaluate(self.gold_file, f.name, self.verbose))
    ...
    def _yield_prediction(self):
        for tokens, (heads, deprels) in zip(utils.conllu.read_conll(self.gold_file), self._outputs):
